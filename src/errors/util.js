...
export function formatWarn(msg: string[], trace: string[]): string {
  return ["Introversion Warning", "", ...msg, "", ...trace].join("\n") + "\n";
}
...
export function _warning(
  ...
  const trace = Stacktrace.getSync()
    ...
    .slice(1);
  format ? warn(formatWarnFmt(msg, trace)) : warn(formatWarn(msg, trace));
}
...
export function warning(
  ...
  _format ? warn(formatWarnFmt(msg, trace)) : warn(formatWarn(msg, trace));
}
